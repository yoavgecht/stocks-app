...
mongodb.MongoClient.connect(dbUrl, (err, database) => {
  ...
  app.post("/api/get-ticker-data", (req, res, next) => {
    ...
    QuandlApi.getRows(req.body.formData, (errorMessage, response) => {
      ...
      //saving the data from the api to lastSearchData object for case user clicks on save button
  ...
  app.post("/api/save-ticker-data", (req, res, next) => {
    ...
    saveDataToDb(lastSearchData);
      ...
      db.listCollections({ name: "savedSearchesDb" })
      .next((err, collinfo) => {
        if (!collinfo) {
          ...
          console.log("created collection", db.collection("savedSearchesDb"));
           ...
           var APIstockBasicInfo = {
             ...
             collection.find({}).toArray((err, data) => {
                if (err) throw err;
                console.log("/api/save-ticker-data data", data);
                res.json(data);
              });
            });
        }
  ...
  const saveDataToDb = () => {
    ...
    collection.insert(lastSearchData.data.dataset, (err, data) => {
      if (err) throw err;
      console.log("saveDataToDb RESPONSE", data);
    });
  }
  ...
  //Calld when user clicks on one of the searched history item
